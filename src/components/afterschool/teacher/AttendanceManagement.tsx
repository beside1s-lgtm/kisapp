...
export const AttendanceManagement: React.FC<AttendanceManagementProps> = ({
  ...
  const handleSaveSubstitute = async () => {
    ...
    const subRecord: SubstituteRecord = {
      id: `sub_${currentCourse.id}_d${subTargetDay.dayIndex}`,
      ...
      originalInstructor: currentCourse.instructorName || '원강사',
            ...
            <table className="w-full border-collapse border border-slate-800 text-xs text-center">
              ...
              <tbody>
                {scheduleDays.map((day) => {
                  ...
                  const effectiveInstructor = subRecord ? subRecord.substituteInstructor : (currentCourse.instructorName || '강사');
                  const effectiveSignature = subRecord ? '' : instructorSignature;

                  return (
                    <tr key={day.dayIndex} className={`h-10 hover:bg-slate-50 ${subRecord ? 'bg-amber-50/40' : ''}`}>
                      ...
                      <td className="border border-slate-800 font-mono text-[11px] text-indigo-900 px-2 py-1">
                        ...
                        {subRecord && (
                          <div className="text-[10px] text-amber-800 font-sans font-medium">사유: {subRecord.reason || '결강 대강'}</div>
                        )}
                      </td>
                      <td className="border border-slate-800 px-2 py-1">
                        {hasChecked ? (
                          ...
                          <div className="flex items-center justify-center gap-1 text-slate-400">
                            {subRecord && <span className="text-[10px] text-amber-700 font-bold">[보결: {subRecord.substituteInstructor}]</span>}
                            <span>미출근 (체크 전)</span>
                          </div>
                        )}
                      </td>
                      <td className="border border-slate-800 px-1 py-1">
                        <button
                          onClick={() => handleOpenSubstituteModal(day)}
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-amber-600 px-6 py-4 flex items-center justify-between text-white">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-base">결강 보결(대강) 강사 등록</h3>
              </div>
              <button
            ...
            <div className="p-6 space-y-4 text-sm">
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3.5 space-y-1">
                ...
                <div className="text-xs text-amber-800">
                  원 담당 강사: <span className="font-bold">{currentCourse.instructorName || '강사'}</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-slate-500">
                  ※ 보결 강사로 등록 시 출근부 해당 회차에 보결자 도장이 날인되며, 학기말 강사료 정산 시 수당이 보결 강사에게 책정됩니다.
                </p>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                {substituteRecords.some(s => s.courseId === currentCourse.id && s.dayIndex === subTargetDay.dayIndex) && (
                  <button
                    onClick={() => {
                      const existing = substituteRecords.find(s => s.courseId === currentCourse.id && s.dayIndex === subTargetDay.dayIndex);
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-1.5"
                >
