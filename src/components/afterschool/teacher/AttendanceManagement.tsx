...
export const AttendanceManagement: React.FC<AttendanceManagementProps> = ({
  ...
  useEffect(() => {
    getUsersDirectory().then(users => {
      if (users && Array.isArray(users)) setAllUsers(users as UserProfile[]);
    }).catch(console.error);
  ...
  const getInstructorSeal = (instName: string): string => {
    ...
    const docSigs = (targetApprovalDoc as any)?.instructorSignatures;
    if (docSigs && docSigs[cleanName]) return docSigs[cleanName];
    if (currentCourse?.instructorName === cleanName && (targetApprovalDoc as any)?.instructorSignature) {
      return (targetApprovalDoc as any).instructorSignature;
    }
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-3 bg-slate-50 border-b border-slate-200 space-y-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 w-full min-w-0">
              ...
              {/* 날짜(회차) 중심 버튼 스크롤 */}
              <div ref={sessionScrollRef} className="flex-1 min-w-0 flex items-center gap-1.5 sm:gap-2 overflow-x-auto overscroll-x-contain pb-0.5 scroll-smooth">
                ...
                return (
                  <div key={enrollment.id} className="w-full px-3.5 py-3 flex items-center justify-between gap-3 bg-white hover:bg-slate-50 transition overflow-x-hidden">
                    <button
                      ...
                      className="flex items-center gap-2.5 flex-1 min-w-0 text-left cursor-pointer group overflow-hidden"
                    >
