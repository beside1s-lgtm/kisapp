...
import { addBus, deleteBus, updateBus, addRoute } from '@/lib/kisbus';
import type { Bus, NewBus, DayOfWeek, RouteType, Destination, Route } from '@/lib/kisbus/types';
...
export const BusRegistrationTab = ({ buses, routes, destinations, semesterMode }: BusRegistrationTabProps) => {
    ...
    const handleAddBusLogic = async (busData: NewBus) => {
        try {
            const newBus = await addBus({ ...busData, semesterMode });
