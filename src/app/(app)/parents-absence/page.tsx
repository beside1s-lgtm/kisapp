...
import { CalendarOff, Loader2, User, Calendar, FileText, ChevronRight } from "lucide-react";
...
export default function ParentsAbsencePage() {
    ...
    useEffect(() => {
        if (user?.uid && profile?.email) {
            getParentDocuments('absence').then(data => {
                setDocs(data);
                setLoading(false);
            });
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
            <div className="flex flex-col gap-2">
                ...
                <p className="text-muted-foreground">학부모님이 제출한 결석계 목록입니다.</p>
            </div>
                                        ...
                                        <div className="space-y-2 text-sm bg-muted/30 p-3 rounded-md">
                                            <div className="flex items-start gap-2">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-medium">결석 기간 ({formData?.absencePeriod?.totalDays || 0}일)</span>
                                                    <span className="text-muted-foreground text-xs">
                                                        {formData?.absencePeriod?.startDate} ~ {formData?.absencePeriod?.endDate}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="flex items-start gap-2 pt-1 border-t border-border/50">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="font-medium">사유: {formData?.absenceType}</span>
